...
func _ready():
	if not InputMap.has_action("toggle_light"):
		InputMap.add_action("toggle_light")

	var key_event := InputEventKey.new()
	key_event.physical_keycode = KEY_E  # For physical location of 'E' key
	InputMap.action_add_event("toggle_light", key_event)
...
func charge(delta):
	...
	if(force_pause or battery == battery_max): return
	battery += delta / 2
